...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        Pattern rPattern = Pattern.compile("(?<=N)[^.;]*?(?=-)");
        Matcher rMatcher = rPattern.matcher(str);

        Pattern pPattern = Pattern.compile("(?<=-)[^.;]*?(?=;)");
        Matcher pMatcher = pPattern.matcher(str);

        Pattern llPattern = Pattern.compile("(?<=;).*?(?=;)");
        Matcher llMatcher = llPattern.matcher(str);
        ...
        if(rMatcher.find()) {
            routeNumber = rMatcher.group(0);
        }

        if(pMatcher.find()) {
            patternName = pMatcher.group(0);
        }

        while(llMatcher.find()){
            double lat = Double.parseDouble(llMatcher.group(0));
            llMatcher.find();
            double lon = Double.parseDouble(llMatcher.group(0));
